...
class Fila {
    ...
    private Celula primeiro; // Remove do indice "primeiro".
    private Celula ultimo; // Insere no indice "ultimo".
    private int num; // Numero de elementos no array.
    ...
    public int media(){ 
      ...
      if (primeiro == 0) {
          for (int i=primeiro; i<ultimo; i++) {
            soma = soma + this.array[i].getDespesaDocente();
          }
      }
      else
      {
          for (int i=primeiro; i<=5; i++) {
            soma = soma + this.array[i].getDespesaDocente();
          }
      ...
      return((int)Math.round(soma/num));
    }
...
public class ex03{
  ...
  public static void main(String[] args) throws Exception{
    ...
    while(num!=0){
      fila.inserir(getInstituicao(num));      
      num = MyIO.readInt();                      
    }
    ...
    for ( int i=0; i<n; i++) {      
      ...
      switch(info[0]){
        case "I":        
          fila.inserir(getInstituicao(Integer.parseInt(info[1])));
          break;
        case "R":
          fila.remover();
          break;                                          
